...
# get monitors from unit and buffer...NOTE: no sites on Wayne, so selecting nearest monitor
...
unit_nadp <- find_nearest_NADP("midewin")
...
ph_plot <- ggplot(data = ph_dat) +
  ...
  geom_line(aes(yr, pH, color = "Shabbona NADP", fill = "Shabbona NADP"), size = 1.2, show.legend = FALSE, data = unit_mons) +
  scale_color_manual(name = NULL,
                    values = c("Shabbona NADP" = "darkgreen", "200km buffer" = "grey85"), breaks = c("Shabbona NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
                    values = c("200km buffer" = "grey75", "Shabbona NADP" = "white"), breaks = c("Shabbona NADP", "200km buffer")) +
  scale_x_continuous(limits = c(1977, 2018),
                     breaks = seq(1977, 2017, 5),
                     minor_breaks = seq(1977, 2017, 1),
                     labels = seq(1977, 2017, 5)) +
  scale_y_continuous(lim = c(4, 5.75), breaks = seq(4,5.75, 0.25), labels = seq(4,5.75, 0.25)) +
...
ggsave(filename = "figures/midewin_ph_trend.jpg",
       plot = ph_plot,
       height = 3,
       width = 4.2,
       units = "in")
...
so4_plot <- ggplot(data = so4_dat) +
  ...
  geom_line(aes(yr, mean_value, color = "200km buffer", fill = "200km buffer"),  size = 1.2, show.legend = FALSE) +
  theme_minimal() +
  geom_line(aes(yr, SO4, color = "Shabbona NADP", fill = "Shabbona NADP"), size = 1.2, data = unit_mons, show.legend = FALSE) +
  scale_color_manual(name = NULL,
                     values = c("Shabbona NADP" = "darkgreen", "200km buffer" = "grey85"),
                     breaks = c("Shabbona NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
                    values = c("200km buffer" = "grey75", "Shabbona NADP" = "white"),
                    breaks = c("Shabbona NADP", "200km buffer")) +
  labs(x = "Year", 
...
ggsave(filename = "figures/midewin_so4_trend.jpg",
       plot = so4_plot,
       height = 3,
       width = 4.2,
       units = "in")
...
n_plot <- ggplot(data = n_dat) +
  ...
  theme_minimal() +
  geom_line(aes(yr, totalN, color = "Shabbona NADP", fill = "Shabbona NADP"), size = 1.2, data = unit_mons) +
  scale_color_manual(name = NULL,
                     values = c("Shabbona NADP" = "darkgreen", "200km buffer" = "grey85"),
                     breaks = c("Shabbona NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
...
ggsave(filename = "figures/midewin_n_trend.jpg",
       plot = n_plot,
       height = 3,
       width = 4.2,
       units = "in") 
