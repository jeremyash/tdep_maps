...
unit_nadp <- find_nearest_NADP("wayne_NF")
...
ph_plot <- ggplot(data = ph_dat) +
  ...
  geom_line(aes(yr, pH, color = "Caldwell NADP", fill = "Caldwell NADP"), size = 1.2, show.legend = FALSE, data = unit_mons) +
  scale_color_manual(name = NULL,
                    values = c("Caldwell NADP" = "darkgreen", "200km buffer" = "grey85"), breaks = c("Caldwell NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
                    values = c("200km buffer" = "grey75", "Caldwell NADP" = "white"), breaks = c("Caldwell NADP", "200km buffer")) +
...
ggsave(filename = "figures/wayne_ph_trend.jpg",
       plot = ph_plot,
       height = 3,
       width = 4.2,
       units = "in")
...
so4_plot <- ggplot(data = so4_dat) +
  ...
  geom_line(aes(yr, mean_value, color = "200km buffer", fill = "200km buffer"),  size = 1.2, show.legend = FALSE) +
  theme_minimal() +
  geom_line(aes(yr, SO4, color = "Caldwell NADP", fill = "Caldwell NADP"), size = 1.2, data = unit_mons, show.legend = FALSE) +
  scale_color_manual(name = NULL,
                     values = c("Caldwell NADP" = "darkgreen", "200km buffer" = "grey85"),
                     breaks = c("Caldwell NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
                    values = c("200km buffer" = "grey75", "Caldwell NADP" = "white"),
                    breaks = c("Caldwell NADP", "200km buffer")) +
  labs(x = "Year", 
...
ggsave(filename = "figures/wayne_so4_trend.jpg",
       plot = so4_plot,
       height = 3,
       width = 4.2,
       units = "in")
...
n_plot <- ggplot(data = n_dat) +
  ...
  theme_minimal() +
  geom_line(aes(yr, totalN, color = "Caldwell NADP", fill = "Caldwell NADP"), size = 1.2, data = unit_mons) +
  scale_color_manual(name = NULL,
                     values = c("Caldwell NADP" = "darkgreen", "200km buffer" = "grey85"),
                     breaks = c("Caldwell NADP", "200km buffer")) +
  scale_fill_manual(name = NULL,
...
ggsave(filename = "figures/wayne_n_trend.jpg",
       plot = n_plot,
       height = 3,
       width = 4.2,
       units = "in") 
