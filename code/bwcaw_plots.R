...
bw <- readOGR("gis/bwcaw")
bw <- spTransform(bw, CRS("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"))
bw_tidy <- tidy(bw)
...
sn_13_15_bw <- crop(sn_13_15, extent(256278, 459408, 2747709, 2834176))


#convert value to polygon
var_poly <- rasterToPolygons(sn_13_15_bw)

#crop value to wisc
var_crop <- crop(var_poly, bw)

#transform dist.crop into a dataframe for plotting
var_crop@data$id <- rownames(var_crop@data)
var_points <- fortify(var_crop, region="id")
var_df <- join(var_points, var_crop@data, by="id")


plot_limits <- var_df %>% 
  summarise(min_dep = min(layer),
            max_dep = max(layer))

ggplot(var_df) +
  geom_polygon(aes(long, lat, group = group, fill = layer), color=NA) +
  scale_fill_viridis(name = "S + N (eq/ha)",
                     limits = c(200,450)) + 
  geom_polygon(data = bw_tidy, aes(long, lat, group = group), 
               fill = NA, size = 1.2, color = "black") +
  coord_equal() +
  theme_map() +
  labs(title = "Mean Sulfur + Nitrogen Wet Deposition, 2013-2015") +
...
ggsave("figures/bwcaw_sn_13_15.pdf", height = 8.5, width = 11, units = "in")
...
lapply(years, function(x) sn_plot_func(x))
