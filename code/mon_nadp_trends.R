...
mon_buffer <- gBuffer(mon, width = 1e+05)

mon <- spTransform(mon, CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))
mon_buffer <- spTransform(mon_buffer, CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))
...
site_sp_points <- spTransform(site_sp_points, proj4string(mon))
...
buffer_mons <- data_frame(siteID = names(over(site_sp_points, mon_buffer)),
                          in_buffer = over(site_sp_points, mon_buffer)) %>% 
